import numpy as np
from PyAstronomy import pyasl
import matplotlib.pyplot as plt
import matplotlib.animation as animation
from matplotlib.animation import PillowWriter
import matplotlib

import json

import os
import pathlib
from datetime import date
import datetime
import os
import glob



writer = PillowWriter(fps=60)

asteroid_name = "testingAsteroid"


#semi-major axis
a = 2.36
#periapsis
per = 2.15
#eccentricity 
e = 0.0894
#longitude of ascending node 
omega = 103.71
#inclination 
i = 7.1422
#argument of periapsis/perifocus 
w = 151.66

#moon orbit
a_moon = 0.00257
per_moon = 5.997
e_moon = 0.0554     # Eccentricity
i_moon = 5.16       # Inclination in degrees
omega_moon = 125.08 # Longitude of ascending node in degrees
w_moon = 318.15     # Argument of periapsis in degrees

moon_orbit = pyasl.KeplerEllipse(a=float(a_moon), e=float(e_moon), per=float(per_moon), i=float(i_moon), Omega=float(omega_moon), w=float(w_moon))
t_moon = np.linspace(0, 4, 200)
pos_moon = moon_orbit.xyzPos(t_moon)


plt.style.use('dark_background')
fig, ax = plt.subplots()


#----------------------------

#define y-unit to x-unit ratio
ratio = 1.0
fig, ax = plt.subplots()

#get x and y limits
x_left, x_right = ax.get_xlim()
y_low, y_high = ax.get_ylim()

#set aspect ratio
ax.set_aspect(abs((x_right-x_left)/(y_low-y_high))*ratio)
ax.set_facecolor('xkcd:black')

a2 = 1.00000011
e2 = 0.01671022
omega2 = 18.272
i2 = 0.00005
w2 = 85.901

per2 = a2 * (1-e2)

orbit2 = pyasl.KeplerEllipse(a=float(a2), per=float(per2), e=float(e2), Omega=float(omega2), i=float(i2), w=float(w2))
t2 = np.linspace(0, 4, 200)

pos2 = orbit2.xyzPos(t2)

# plt.plot(pos2[::, 1], pos2[::, 0], 'k-', label="Earth Trajectory", color="yellow", linewidth=1)
# plt.plot(pos2[0, 1], pos2[0, 0], 'r*', label="Earth Periapsis", color="blue", linewidth=1)

plt.plot(0, 0, 'bo', markersize=9, label="Sun", color="yellow", linewidth=1)

#moon orbit plot relative to earth orbit
pos_moon_relative = np.array([pos_moon[j] + pos2[j] for j in range(len(t_moon))])
# print("moon relative", pos_moon_relative)
# print("earth", pos2)

plt.plot(pos_moon_relative[::, 1], pos_moon_relative[::, 0], 'k-', label="Moon Trajectory", color="white", linestyle="dotted", linewidth=1)
plt.plot(pos_moon_relative[0, 1], pos_moon_relative[0, 0], 'r*', label="Moon Periapsis", color="white", linewidth=1)

plt.grid(linewidth = 0.2)

print("-----------------------Checkpoint1--------------------------")


#----------------------------




red_dot2, = plt.plot(pos2[0][1], pos2[0][0], 'bo', color="blue")

red_dot3, = plt.plot(pos_moon_relative[0][1], pos_moon_relative[0][0], 'bo', color="white")


ln, = plt.plot([], [], 'bo-', animated=True, color="#fe28a2", linewidth = 0.6)

# ln2, = plt.plot([], [], 'bo-', animated=True, color="#00b2ee", linewidth = 0.6)

# ln3, = plt.plot([], [], 'bo-', animated=True, color="#bcbcbc", linewidth = 0.6)


def animate(i):
    

    red_dot2.set_data(pos2[i][1], pos2[i][0])
    red_dot3.set_data(pos_moon_relative[i][1], pos_moon_relative[i][0])

    ln.set_data([pos_moon_relative[i][1], pos2[i][1]], [pos_moon_relative[i][0], pos2[i][0]])

    # ln2.set_data([0, pos2[i][1]], [0, pos2[i][0]])

    # ln3.set_data([0, pos[i][1]], [0, pos[i][0]])

    return ln, red_dot2, red_dot3


# create animation using the animate() function
#make the animation longer so it encapsulates the entire orbit

myAnimation = animation.FuncAnimation(fig, animate,
    interval=40,
    frames=np.arange(0, 200),
    blit=True,
    repeat=True)


plt.tick_params(
    axis='both', 
    which='both', 
    bottom=True, 
    top=False, 
    labelbottom=True,  
    left=True,
    labelleft=True)

plt.legend(loc="lower right", fontsize='xx-small')

plt.title(f'{asteroid_name} Orbital Simulation')


print("-----------------------Checkpoint2--------------------------")
try:
    myAnimation.save(f'animated_{asteroid_name}.gif', writer=writer)
except:
    pass
print("-----------------------Checkpoint3--------------------------")

plt.show()